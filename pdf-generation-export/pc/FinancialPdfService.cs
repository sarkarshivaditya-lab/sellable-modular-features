...
public static class FinancialPdfService
{
        ...
        AppSettings settings)
    {
        ...
        Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Element(c => ComposeContent(c, incomeEntries, expenses, totalIncome, totalExpenses, net));
    ...
    {
        var centreName = string.IsNullOrWhiteSpace(settings.CentreName) ? "Urban Diagnostic Centre" : settings.CentreName;
        c.BorderBottom(1).BorderColor("#CFD8DC").PaddingBottom(12).Column(col =>
        {
            ...
            col.Item().AlignCenter().Text("FINANCIAL SUMMARY REPORT").FontSize(13).Bold().FontColor("#1A237E").LetterSpacing(1);
            col.Item().PaddingTop(6).AlignCenter().Text($"Reporting Period:  {from:dd MMM yyyy} – {to:dd MMM yyyy}").FontSize(10).FontColor("#546E7A");
        });
    }

    private static void ComposeContent(IContainer c, List<IncomeEntry> incomeEntries, List<FinancialTransaction> expenses, decimal totalIncome, decimal totalExpenses, decimal net)
    {
        c.PaddingTop(16).Column(col =>
        {
            col.Item().Background("#F5F7FA").Border(1).BorderColor("#CFD8DC").Padding(14).Column(box =>
            {
                box.Item().Text("FINANCIAL OVERVIEW").FontSize(8).Bold().FontColor("#757575").LetterSpacing(1);
                box.Item().PaddingTop(8).Table(t =>
                {
                    ...
                    SummaryRow(t, "Net Profit / Loss", $"₹ {net:N2}", net >= 0 ? "#1B5E20" : "#B71C1C");
                });
            });

            col.Item().PaddingTop(20).Text("INCOME SUMMARY").FontSize(8).Bold().FontColor("#757575").LetterSpacing(1);
            if (incomeEntries.Count == 0)
                col.Item().PaddingTop(6).Text("No income recorded for this period.").FontSize(10).FontColor("#9E9E9E").Italic();
            else
                col.Item().PaddingTop(6).Table(t =>
                {
                    t.ColumnsDefinition(cols =>
                    {
                        cols.RelativeColumn(1.2f); cols.RelativeColumn(2); cols.RelativeColumn(1.5f); cols.RelativeColumn(1);
                    });
            ...
            col.Item().PaddingTop(20).Text("EXPENSE SUMMARY").FontSize(8).Bold().FontColor("#757575").LetterSpacing(1);
            if (expenses.Count == 0)
                col.Item().PaddingTop(6).Text("No expenses recorded for this period.").FontSize(10).FontColor("#9E9E9E").Italic();
            else
            {
                var byCategory = expenses.GroupBy(e => e.Category).OrderBy(g => g.Key).Select(g => (Category: g.Key, Total: g.Sum(e => e.Amount))).ToList();
                col.Item().PaddingTop(6).Table(t =>
                {
            ...
            col.Item().PaddingTop(20).Text("TRANSACTION LEDGER (CHRONOLOGICAL)").FontSize(8).Bold().FontColor("#757575").LetterSpacing(1);
            var allRows = BuildLedger(incomeEntries, expenses);
            if (allRows.Count == 0)
                col.Item().PaddingTop(6).Text("No transactions in this period.").FontSize(10).FontColor("#9E9E9E").Italic();
            else
                col.Item().PaddingTop(6).Table(t =>
                {
                    t.ColumnsDefinition(cols =>
                    {
                        cols.RelativeColumn(1.2f); cols.RelativeColumn(0.8f); cols.RelativeColumn(1.2f); cols.RelativeColumn(2.5f); cols.RelativeColumn(1);
                    });
                    ...
                    foreach (var row in allRows)
                    {
                        ...
                        t.Cell().Background(bg).Padding(5).Text(row.Type).FontSize(8).FontColor(amtColor);
                        ...
                        t.Cell().Background(bg).Padding(5).Text($"₹ {row.Amount:N2}").FontSize(8).Bold().FontColor(amtColor);
                    }
                });
        });
    }
    ...
    private static List<(DateTime Date, string Type, string Category, string Description, decimal Amount)> BuildLedger(List<IncomeEntry> income, List<FinancialTransaction> expenses)
    {
        var rows = new List<(DateTime, string, string, string, decimal)>();
        foreach (var e in income) rows.Add((e.Date, "Income", "Diagnostic Revenue", $"{e.PatientName} — {e.ReportCode}", e.Amount));
        foreach (var e in expenses) rows.Add((e.TransactionDate, "Expense", e.Category, e.Description, e.Amount));
    ...
    {
        foreach (var h in headers) t.Cell().Background("#1565C0").Padding(7).Text(h).FontSize(9).Bold().FontColor("#FFFFFF");
    }

    private static void TotalRow(TableDescriptor t, int emptySpan, string total)
    {
        t.Cell().ColumnSpan((uint)emptySpan).Background("#E8EAF6").Padding(7).AlignRight().Text("Total:").FontSize(10).Bold().FontColor("#1A237E");
        t.Cell().Background("#E8EAF6").Padding(7).Text(total).FontSize(10).Bold().FontColor("#1A237E");
    }
}
