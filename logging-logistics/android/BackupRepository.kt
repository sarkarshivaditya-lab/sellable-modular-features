...
class BackupRepository @Inject constructor(
    ...
    suspend fun exportDatabaseFile(uri: Uri): BackupResult = withContext(Dispatchers.IO) {
        runCatching {
            ...
            context.contentResolver.openOutputStream(uri)?.use { out -> dbFile.inputStream().use { it.copyTo(out) } }
                ?: return@withContext BackupResult.Error("Could not open output stream")
            BackupResult.Success("Database exported successfully")
        }.getOrElse { BackupResult.Error("Export failed: ${it.localizedMessage}") }
    }
    ...
    suspend fun importDatabaseFile(uri: Uri): BackupResult = withContext(Dispatchers.IO) {
        runCatching {
            val dbFile = context.getDatabasePath(AppDatabase.DATABASE_NAME)
            database.close()
            context.contentResolver.openInputStream(uri)?.use { input -> FileOutputStream(dbFile).use { input.copyTo(it) } }
        ...
        }.getOrElse { BackupResult.Error("Import failed: ${it.localizedMessage}") }
    }

    suspend fun exportToJson(uri: Uri): BackupResult = withContext(Dispatchers.IO) {
        runCatching {
            val customers = database.patientDao().getAllPatients().first()
            val services = database.labTestDao().getAllTests().first()
            val backup = BackupData(
                exportedAt = LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME),
                patients = customers,
                labTests = services
            )
            context.contentResolver.openOutputStream(uri)?.use { it.write(gson.toJson(backup).toByteArray()) }
                ?: return@withContext BackupResult.Error("Could not open output stream")
            BackupResult.Success("Exported ${backup.patients.size} customers and ${backup.labTests.size} services")
        }.getOrElse { BackupResult.Error("Export failed: ${it.localizedMessage}") }
    }
    ...
    suspend fun importFromJson(uri: Uri): BackupResult = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val backup: BackupData = gson.fromJson(json, object : TypeToken<BackupData>() {}.type)
            ...
            BackupResult.Success("Imported ${backup.patients.size} customers and ${backup.labTests.size} services")
        }.getOrElse { BackupResult.Error("Import failed: ${it.localizedMessage}") }
    }

    suspend fun exportPatientsToCSV(uri: Uri, filter: CsvFilter): BackupResult = withContext(Dispatchers.IO) {
        runCatching {
            val allCustomers = database.patientDao().getAllPatients().first()
            val today = DateTimeFormatter.ISO_LOCAL_DATE.format(LocalDate.now())
            val customers = when (filter) {
                CsvFilter.TODAY -> allCustomers.filter { it.date == today }
                CsvFilter.ALL -> allCustomers
                CsvFilter.PENDING -> allCustomers.filter { it.paymentStatus in listOf("UNPAID", "PARTIAL") }
            }
            val serviceType = object : TypeToken<List<SelectedTest>>() {}.type
            val header = "Receipt No,Customer No,Name,Phone,Date,Services,Subtotal,Discount,Grand Total,Payment Status,Payment Method,Amount Received,Outstanding\n"
            context.contentResolver.openOutputStream(uri)?.bufferedWriter()?.use { writer ->
                writer.write(header)
                customers.forEach { p ->
                    val services = runCatching { gson.fromJson<List<SelectedTest>>(p.selectedTests, serviceType) }.getOrDefault(emptyList())
                    val serviceNames = services.joinToString("; ") { it.testName }.csv()
                    val outstanding = p.grandTotal - p.amountReceived
                    writer.write("${p.receiptNumber.csv()},${p.patientNumber.csv()},${p.name.csv()},${p.phone.csv()},${p.date},$serviceNames,${p.subtotal},${p.discountValue},${p.grandTotal},${p.paymentStatus},${p.paymentMethod},${p.amountReceived},$outstanding\n")
                }
            } ?: return@withContext BackupResult.Error("Could not open output stream")
            BackupResult.Success("Exported ${customers.size} records to CSV")
        }.getOrElse { BackupResult.Error("CSV export failed: ${it.localizedMessage}") }
    }
